...
import logging
import warnings
from new_data_loader_classify import load_custom_dataset  # 确保导入新的数据加载函数
...
dataset_list = {
    'train_data': {
        'name': 'TrainData',
        'train_size': '83897',  # 更新为实际数据集大小
        'test_size': '0',  # 
        'dims': '16',  # 更新为实际特征维度
        'length': '1',  # 如果每条记录是独立的，可以设置为 1
        'classes': '',  # 更新为实际类别数
        'batch_size': '32'  # 根据需要设置批次大小
    }
}
...
parser1.add_argument('--target-dataset', type=str, default='train_data', help='None')
...
parser.add_argument('--cls-loss-weight', type=float, default=10, metavar='LR', help='classification loss weight (default: 0.1)')
...
parser.add_argument('--pre-train-weight', type=str, default='./classify_'+ machine_idx +'/HMM_model_' + target_dataset + '_pretrain.pkl', help='VAE-pre-train-weight-path')
...
def main():
    ...
    # 使用新的数据加载函数
    train_loader = load_custom_dataset('train_data.csv', batch_size=batch_size, shuffle=True)
    val_loader = load_custom_dataset('trade_data.csv', batch_size=batch_size, shuffle=False)

    n_iter = 0
    for epoch in range(train_epoch):
        print('training in epoch {0}'.format(epoch))
        n_iter = train(args, hmm_model, train_loader, batch_size, optimizer_hmm, writer, epoch, n_iter, train_log)

        scheduler_hmm.step()

        acc_val_value = val(args, hmm_model, val_loader, batch_size, writer, epoch, n_iter, val_log)
